...
User.where(:email => 'moinhaidar@gmail.com').first_or_create(
  ...
  :date_of_birth => 20.years.ago.to_s(:db),
  :role => 'admin',
  :designation_id => Designation.find_by_name('Team Lead').id,
  :manager_id => User.first.id
)
ProjectsUser.create(:user_id => User.first.id, :project_id => Project.first.id)
ProjectsUser.create(:user_id => User.last.id, :project_id => Project.last.id)
